...
1. computes the prompt-semhash similarity digest and stores it on the observable as an
...
class PromptCorrelationConnector:
    ...
    def _process_message(self, data: dict) -> str:
        ...
        matches = find_similar(value, self._candidate_digests(observable["id"]), self.threshold)

        # 3. link the similar ones
        for candidate_id, score in matches:
            self.helper.api.stix_core_relationship.create(
                fromId=observable["id"],
                toId=candidate_id,
                relationship_type="related-to",
                description=f"prompt-semhash similarity {score:.2f}",
